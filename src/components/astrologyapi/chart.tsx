 ...
 */
export function AstrologyApiChart({ houses }: { houses: AstrologyApiChartHouse[] }) {
  return (
    <Card>
        ...
        {houses.map((house, i) => (
          <div key={house.sign} className="rounded-lg border border-border bg-muted/30 p-3">
            <div className="mb-2 flex items-center justify-between">
              <span className="text-xs font-medium text-muted-foreground">House {i + 1}</span>
              <span className="text-xs text-muted-foreground">{house.sign_name}</span>
            </div>
